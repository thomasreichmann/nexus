...
export async function GET(): Promise<NextResponse> {
    ...
    for (const m of METRICS) {
        total[m] = {
            ...agg[m],
            pct: agg[m].total > 0 ? (agg[m].covered / agg[m].total) * 100 : 0,
        };
    }

    // E2E coverage report, generated by `pnpm -F web e2e:coverage`.
    let e2e: unknown = null;
    try {
        e2e = JSON.parse(
            readFileSync(
                join(root, 'apps/web', 'coverage', 'e2e-coverage.json'),
                'utf8'
            )
        );
    } catch {
        // Missing report → page shows how to generate it.
    }

    return NextResponse.json({ workspaces, total, e2e });
}
